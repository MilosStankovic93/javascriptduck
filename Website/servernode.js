...
app.post("/zakazi", function(req, res){
	...
	con.query("INSERT INTO zakazi (zak_ime, zak_prezime, zak_vreme, zak_datum) VALUES (?,?,?,?)",
		[zak_ime, zak_prezime, zak_vreme, zak_datum], function(err, result, field){
			if(err){
				res.json({
					result: "ERROR"
				})
...
app.get("/zakazi", function(req, res){
	var zak_ime = req.query.ime;
	var zak_prezime = req.query.prezime;
	var zak_vreme = req.query.vreme;
	var zak_datum = req.query.datum

	con.query("SELECT DATE_FORMAT(zak_vreme,'%k:%i') AS 'zak_vreme', DATE_FORMAT(zak_datum, '%Y-%m-%d') AS 'zak_datum' FROM zakazi", function(err, result, field){
			if(err) throw err;
				console.log(result);
				res.json({
					data: result
